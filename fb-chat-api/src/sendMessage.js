...
module.exports = function (defaultFuncs, api, ctx) {
	...
	function uploadAttachment(attachments, callback) {
		...
		// Create an array of promises for all attachments
		for (var i = 0; i < attachments.length; i++) {
			...
			var form = {
				...
				voice_clip: "true" // You can add conditions for other types if needed
			};
	...
	// Get URL attachment for sharing
	...
	// Handle message types like URL, Location, Sticker, Emoji, Attachments, Mentions
	...
	function handleEmoji(msg, form, callback, cb) {
		...
		if (msg.emoji) {
			...
			if (msg.emojiSize != "small" && msg.emojiSize != "medium" && msg.emojiSize != "large") return callback({ error: "Invalid emojiSize." });
			if (form["body"] != null && form["body"] != "") return callback({ error: "Body should be empty when emoji is set." });
			form["body"] = msg.emoji;
